...
- **SEO**: per-page `generateMetadata` sets localized title/description plus `alternates` (canonical + `hreflang` en/de/x-default via `lib/seo.ts`). `app/sitemap.ts` and `app/robots.ts` cover both locales. The OG card is a static `app/[locale]/opengraph-image.png` (with `opengraph-image.alt.txt`), shared by both locales; the former dynamic `opengraph-image.tsx` (locale-specific card text via `next/og`) was removed.
